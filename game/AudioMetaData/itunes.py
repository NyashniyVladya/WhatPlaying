...
class ITunesWebParser(object):
    ...
    def web_tag(self):
        ...
        _title = self._audio._get_basic_tag("title_tag", ignore_web_tag=True)
        _album = self._audio._get_basic_tag("album_tag", ignore_web_tag=True)
        for variant in map(WebTag, variants):
            if _title.lower() == variant.title_tag.lower():
                if _album and (_album.lower() != variant.album_tag.lower()):
                    continue
    ...
    def _get_results_about_track(self):
        ...
        with self.search(term=term) as webpage:
            result = json.load(webpage, encoding="utf_8")
